...
/**
Deklaracja klasy Map
- Odpowiednik STLowego mapy/s³ownika.
...
class Map : public Associative<T>
{
...
public:
	...
	*/
	Map() 
	{
		...
		else
		{
			...
			key = nullptr;
			data = nullptr;
			cerr << "\n[Map<" << showTypeKey() << "><" << showTypeValue() << ">] " << "[Konstruktor domyœlny] Niedozwolony typ kontenera!" << endl;
		}
	}
	...
	Map(T key, V value)
	{
		if (allowedType())
		{
			...
			if (key == returnEmptyKey() || value == returnEmptyValue())
			{
				...
				this->key = nullptr;
				this->data = nullptr;
				cerr << "\n[Map<" << showTypeKey() << "><" << showTypeValue() << ">] " << "[Konstruktor dwuargumentowy] Element nie mo¿e byæ pusty!" << endl;
			}
		...
		else
		{
			...
			this->key = nullptr;
			this->data = nullptr;
			cerr << "\n[Map<" << showTypeKey() << "><" << showTypeValue() << ">] " << "[Konstruktor dwuargumentowy] Niedozwolony typ kontenera!" << endl;
		}
	}
	...
	*/
	...
	{
		//Wielkoœci i pojemnoœci obu Map s¹ równe
		if (size == other.Size())
		{
	...
	*/
	void PopBack()
	{
		...
		{
			cerr << "\n[Map<" << showTypeKey() << "><" << showTypeValue() << ">] " << e.what() << endl;
		}
	}
	...
	*/
	...
	@param key Podany klucz do sprawdzenia.
	@return Status operacji: true, je¿eli jest unikatowy; false jeœli nie.
	*/
	bool usedKey(const T& key)
	{
		if (key != returnEmptyKey())
		{
			for (int i = 0; i < size; i++)
			{
				if (key == this->key[i])
	...
	*/
	void Insert(const T& key, const V& value)
	{
		try
		{
			...
			{
				//Wartoœæ nie jest pusta
				if (value != returnEmptyValue() && !usedKey(key))
				{
					T* tmp1 = new T[size + 1];
					V* tmp2 = new V[size + 1];
					for (int i = 0; i < size; i++)
