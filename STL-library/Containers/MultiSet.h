...
public:
	...
	*/
	...
	{
		//Je¿eli wielkoœæ MultiSety nie wykracza poza zakres
		if (!other.Empty())
		{
	...
	*/
	...
	{
		//Wielkoœci i pojemnoœci obu MultiSet s¹ równe
		if (size == other.Size())
		{
	...
	*/
	...
	/**
	Zwraca index podanê wartoœci o ile jest w kontenerze.

	@param value Wartoœæ do odnalezienia.
	@return Indeks wartoœci o ile znajduje siê w kontenerze, jeœli nie to zwracana jest wartoœæ -1.
	*/
	int Index(T value)
	{
		if (key)
	...
	*/
	void Insert(int position, const T& data)
	{
		try
		{
			...
			//MultiSet wykracza poza skalê
			else
			{
				throw out_of_range("[Insert] MultiSet przekracza maksymaln¹ wielkoœæ!");
			}
		}
		catch (const invalid_argument& e)
		{
	...
	*/
	void Insert(int position, const T& data, int count)
	{
		try
		{
			...
			{
				throw out_of_range("MultiSet przekracza maksymaln¹ wielkoœæ!");
			}
		}
		catch (const invalid_argument& e)
		{
	...
	*/
	void Remove(T value)
	{
		try
		{
			if (size != 0)
			{
				...
				if (!findIfExist(value))
				{
					throw invalid_argument("Wartoœæ do usuniêcia nie istnieje!");
				}
				//Indeks jest poprawny
				else
				{
	...
	/**
	Zliczenie iloœci wyst¹pienieñ klucza w kontenerze.

	@param value Wartoœæ klucza do zliczeia.
