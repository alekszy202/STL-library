...
*/
template <class T>
class ListCircle : public Sequences<T>
{
...
public:
	...
	*/
	ListCircle() : pHead(nullptr)
	{
		if (!allowedType())
		{
			cerr << "\n[ListCircle<" << showType() << ">] " << "[Konstruktor domyslny] Niedozwolony typ!" << endl;
		}
	}
	...
	*/
	ListCircle(T value)
	{
		if (allowedType())
		{
			...
			{
				pHead = nullptr;
				cerr << "\n[ListCircle<" << showType() << ">] " << "[Konstruktor jednoargumentowy] Bledny argument, stworzono pusty kontener!" << endl;
			}
		}
		else
		{
			pHead = nullptr;
			cerr << "\n[ListCircle<" << showType() << ">] " << "[Konstruktor jednoargumentowy] Niedozwolony typ kontenera!" << endl;
		}
	}
	...
	*/
	ListCircle(T value, int count)
	{
		if (allowedType() && value != returnEmpty())
		{
			...
			{
				pHead = nullptr;
				cerr << "\n[ListCircle<" << showType() << ">] " << "[Konstruktor dwuargumentowy] Bledny argument, stworzono pusty kontener!" << endl;
			}
		}
		else
		{
			pHead = nullptr;
			cerr << "\n[ListCircle<" << showType() << ">] " << "[Konstruktor dwuargumentowy] Niedozwolony typ kontenera!" << endl;
		}
	}
	...
	*/
	ListCircle(const ListCircle& other)
	{
		...
		{
			pHead = nullptr;
			cerr << "\n[ListCircle<" << showType() << ">] " << "[Konstruktor kopiujacy] Pusty kontener!" << endl;
		}
	}
	...
	*/
	friend ostream& operator<<(ostream& ostr, const ListCircle& list)
	{
		int size = list.Size();
	...
	void PopBack()
	{
		try
		{
			...
			else
			{
				...
				//Uzupełnienie listy kołowej
				tmp->pPrev->pNext = pHead;
				pHead->pPrev = tmp->pPrev;
		...
		{
			cerr << "\n[ListCircle<" << showType() << ">] " << e.what() << endl;
		}
	}
		...
		try
		{
			...
			if (pHead)
			{
				...
				if (pHead->pNext)
				{
					...
					//Uzupełnienie listy kołowej
					tmp->pPrev = pHead->pPrev;
					pHead->pPrev->pNext = tmp;
		...
		try
		{
			if (value != returnEmpty())
			{
				...
				else if ((Size() + 1) < MaxSize())
				{
					...
					//Uzupełnienie listy kołowej
					tmp->pNext = new ListBiNode<T>(value, pHead, tmp);
					pHead->pPrev = tmp->pNext;
				}
				else
				{
		...
		try
		{
			if (value != returnEmpty())
			{
				...
				if (pHead)
				{
					...
					pHead = new ListBiNode<T>(value, tmp, tmp->pPrev);
					tmp->pPrev = pHead;
	...
	*/
	...
	{
		if (this->pHead) return this->pHead;
		else return nullptr;
	}
		...
		try
		{
			if (value != returnEmpty())
			{
				...
				if ((size + 1) <= MaxSize())
				{
					...
					if (position == 0)
					{
						ListBiNode<T>* newNode = new ListBiNode<T>(value, pHead, pHead->pPrev);
						this->pHead = newNode;

						//Uzupełnienie listy kołowej
						this->pHead->pNext->pPrev = this->pHead;
						this->pHead->pPrev->pNext = this->pHead;
					}
			...
			if (value != returnEmpty() && (n > 0) && (n < MaxSize()))
			{
				...
				if ((size + n) <= MaxSize())
				{
					...
					if (position == 0)
					{
						...
						for (int i = 0; i < n; i++)
						{
							newNode = new ListBiNode<T>(value, pHead, pHead->pPrev);
							pHead = newNode;
		...
		try
		{
			if ((n > 0) && (n < MaxSize()))
			{
				...
				if ((size + n) <= MaxSize())
				{
					...
					if (position == 0)
					{
						ListBiNode<T>* newNode = new ListBiNode<T>(*valuePtr, pHead, pHead->pPrev);
						pHead = newNode;
						pHead->pNext->pPrev = pHead;
						pHead->pPrev->pNext = pHead;
		...
		try
		{
			if (count > 0 && count <= MaxSize())
			{
				...
				else
				{
					...
					throw invalid_argument("Podana lista jest pusta!");
				}
			}
			else
			{
				//Wyjątek w przypadku podania ujemnej liczby
				throw invalid_argument("Podana liczba nie jest liczba dodatnia!");
			}
		}
		catch (const invalid_argument& e)
		{
		...
		try
		{
			...
			if ((position < size1) && (position > -1))
			{
				if (position == 0)
				{
					...
					pHead->pPrev->pNext = x.Begin();
					x.Begin()->pPrev = pHead->pPrev;
	...
	*/
	int Size() const noexcept
	{
		...
		//Sprawdzenie czy istnieje głowa
		if (tmp)
		{
			counter++;
			if (tmp->pNext)
		...
		//Obrót pętlą tak dlugo az wroci do pHead
		while (tmp != pHead)
		{
			counter++;
			tmp = tmp->pNext;
