...
@param pHead Głowa listy jednokierunkowej.
*/
template <class T>
class List : public Sequences<T>
{
	//Głowa listy jednokierunkowej
...
public:
	...
	*/
	List() : pHead(nullptr)
	{
		if (!allowedType())
		{
			cerr << "\n[List<" << showType() << ">] " << "[Konstruktor domyslny] Niedozwolony typ!" << endl;
		}
	}
	...
	*/
	List(T value)
	{
		if (allowedType())
		{
			...
			{
				this->pHead = new ListNode<T>(value, nullptr);
			}
			else
			{
				pHead = nullptr;
				cerr << "\n[List<" << showType() << ">] " << "[Konstruktor jednoargumentowy] Bledny argument, stworzono pusty kontener!" << endl;
			}
		}
		else
		{
			pHead = nullptr;
			cerr << "\n[List<" << showType() << ">] " << "[Konstruktor jednoargumentowy] Niedozwolony typ kontenera!" << endl;
		}
	}
	...
	*/
	List(T value, int count)
	{
		if (allowedType())
		{
			if (value != returnEmpty() && count > 0 && count < MaxSize())
			{
				//Utworzenie glowy i zmiennej pomocniczej
				this->pHead = new ListNode<T>(value, nullptr);
				ListNode<T>* tmp = this->pHead;

				//Pętla uzupelnia liste o pozostale elementy
				for (int i = 0; i < (count - 1); i++)
				{
					tmp->pNext = new ListNode<T>(value, nullptr);
					tmp = tmp->pNext;
				}
			}
			else
			{
				pHead = nullptr;
				cerr << "\n[List<" << showType() << ">] " << "[Konstruktor dwuargumentowy] Bledny argument, stworzono pusty kontener!" << endl;
			}
		}
		else
		{
			pHead = nullptr;
			cerr << "\n[List<" << showType() << ">] " << "[Konstruktor dwuargumentowy] Niedozwolony typ kontenera!" << endl;
		}
	}
	...
	*/
	List(int count, T* valuePtr)
	{
		if (allowedType())
		{
			if (valuePtr != nullptr && count > 0 && count < MaxSize())
			{
				//Utworzenie glowy i zmiennej pomocniczej
				this->pHead = new ListNode<T>(*valuePtr, nullptr);
				ListNode<T>* tmp = this->pHead;
				...
				{
					valuePtr++;
					tmp->pNext = new ListNode<T>(*valuePtr, nullptr);
					tmp = tmp->pNext;
				}
			}
			else
	...
	*/
	List(int count, ListNode::ListNode<T>* ptr)
	{
		if (allowedType())
		{
			...
			{
				//Utworzenie glowy i zmiennej pomocniczej
				this->pHead = new ListNode<T>(ptr->data, nullptr);
				ListNode<T>* tmp = this->pHead;
				ListNode<T>* argument = ptr;
				...
				for (int i = 0; i < (count - 1); i++)	//Iteracja count-1 razy by utworzyc porzadana liczbe elementow
				{
					...
					} while (argument->pNext);

					tmp->pNext = new ListNode<T>(argument->data, nullptr);
					tmp = tmp->pNext;
				}
			}
			else
	...
	*/
	List(List& other)
	{
		if (!other.Empty())
		{
			...
			this->pHead = new ListNode<T>(tmp->data, nullptr);

			ListNode<T>* tmp2 = this->pHead;
			for (int i = 0; i < (size - 1); i++)
			{
				tmp = tmp->pNext;
				tmp2->pNext = new ListNode<T>(tmp->data, nullptr);
				tmp2 = tmp2->pNext;
			}
		}
		else
		{
			pHead = nullptr;
			cerr << "\n[List<" << showType() << ">] " << "[Konstruktor kopiujący] Bledny argument, stworzono pusty kontener!" << endl;
		}
	}
	...
	*/
	bool operator==(List& rhs)
	{
		...
		if (size1 == size2)
		{
			...
			ListNode<T>* ptr1 = this->Begin();
			ListNode<T>* ptr2 = rhs.Begin();
	...
	*/
	friend ostream& operator<<(ostream& ostr, const List& list)
	{
		ListNode<T>* tmp = list.pHead;
		while (tmp != nullptr)
		{
			cout << tmp->data << " ";
			tmp = tmp->pNext;
	...
	void PushFront(const T& value)
	{
		try
		{
			if (value != returnEmpty())
			{
				...
				{
					ListNode<T>* tmp = this->pHead;
					this->pHead = new ListNode<T>(value, tmp);
				}
				else
				{
		...
		try
		{
			if (value != returnEmpty())
			{
				...
				if ((size + 1) <= MaxSize())
				{
					...
					if (position == 0)
					{
						ListNode<T>* newNode = new ListNode<T>(value, pHead);
						this->pHead = newNode;
					}
					else if ((position > 0) && (position < size))
					{
						...
						ListNode<T>* tmp2 = new ListNode<T>(value, tmp->pNext);
						tmp->pNext = tmp2;
					}
					else
					{
			...
			if (value != returnEmpty() && (n > 0) && (n < MaxSize()))
			{
				...
				if ((size + n) <= MaxSize())
				{
					...
					if (position == 0)
					{
						...
						for (int i = 0; i < n; i++)
						{
							newNode = new ListNode<T>(value, pHead);
							pHead = newNode;
						}
					}
					else if ((position > 0) && (position < size))
					{
						...
						for (int j = 0; j < n; j++)
						{
							tmp2 = new ListNode<T>(value, tmp->pNext);
							tmp->pNext = tmp2;
						}
					}
					else
		...
		try
		{
			if ((n > 0) && (n < MaxSize()))
			{
				...
				if ((size + n) <= MaxSize())
				{
					...
					if (position == 0)
					{
						ListNode<T>* newNode = new ListNode<T>(*valuePtr, pHead);
						pHead = newNode;
						ListNode<T>* newNode2 = nullptr;

						for (int i = 0; i < (n - 1); i++)
						{
							valuePtr++;
							newNode2 = new ListNode<T>(*valuePtr, newNode->pNext);
					...
					else if ((position > 0) && (position < size))
					{
						...
						for (int j = 0; j < n; j++)
						{
							tmp2 = new ListNode<T>(*valuePtr, tmp->pNext);
							tmp->pNext = tmp2;
							tmp = tmp2;
							valuePtr++;
						}
		...
		try
		{
			if (count > 0 && count <= MaxSize())
			{
				if (element != nullptr)
				{
					...
					this->pHead = new ListNode<T>(element->data, nullptr);	//Utworzenie głowy
					ListNode<T>* tmp = this->pHead;
					ListNode<T>* argument = element;
					...
					for (int i = 0; i < (count - 1); i++)	//Iteracja count-1 razy by utworzyc porzadana liczbe elementow
					{
						...
						{
							throw out_of_range("[Assign] Podany element listy nie posiada tylu nastepnikow!");
							return;
						}
	...
	void Assign(int count, T* valuePtr)
	{
		try
		{
			if (count > 0 && count <= MaxSize())
			{
				...
				{
					this->Clear();	//Zwolnienie wcześniej istniejącej pamieci
					this->pHead = new ListNode<T>(*valuePtr, nullptr);	//Utworzenie głowy
					ListNode<T>* tmp = this->pHead;

					for (int i = 1; i < count; i++)	//Iteracja count-1 razy by utworzyc porzadana liczbe elementow
					{
						tmp->pNext = new ListNode<T>(*(valuePtr + i), nullptr);
						tmp = tmp->pNext;
					}
				}
				else
	...
	void Assign(int count, T value)
	{
		try
		{
			if (value != returnEmpty())
			{
				...
				{
					this->Clear();	//Zwolnienie wcześniej istniejącej pamieci
					this->pHead = new ListNode<T>(value, nullptr);	//Utworzenie głowy
					ListNode<T>* tmp = this->pHead;
				...
				{
					//Wyjątek w przypadku podania ujemnej liczby
					throw invalid_argument("[Assign] Nieprawidłowy arguemnt!");
				}
			}
			else
			{
	...
	*/
	void Swap(List& l2)
	{
		ListNode<T>* tmp = this->pHead;	//Zmienna tymczasowa przechowująca głowę
	...
	void Erase(int position)
	{
		try
		{
			...
			if ((position < this->Size()) && (position > -1))
			{
				...
				if (position == 0)
				{
					ListNode<T>* tmp = this->pHead->pNext;
					delete this->pHead;
					this->pHead = tmp;
				}
				//gdy inny element
				else
				{
					...
					ListNode<T>* tmp2 = tmp->pNext->pNext;
					delete tmp->pNext;
					tmp->pNext = tmp2;
				}
			}
